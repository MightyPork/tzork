...
var b=e.year();if(b<c){return -1}else{if(b>c){return 1}else{if(d<f){return -1}else{if(d>f){return 1}}}}return 0}var people_loading=0;function loadPeopleArray(d){var b=localStorage.people;if(typeof b!="undefined"){try{people=JSON.parse(b)
}catch(c){console.error("Error when parsing people array from localstorage",c)}}people_loading=0;people.forEach(function(e){e._valid=true;var f=false;["name","color","tz"].forEach(function(g){if(typeof(e[g])=="undefined"){console.error('Missing "'+g+'" field in person object',e);
f=true}});if(f){e._valid=false;return}people_loading++;resolveTimezone(e)});var a=function(){if(people_loading<=0){console.log("LOADING DONE");d();return}setTimeout(a,10)};a()}var last_google_call_timestamp=0;
function resolveTimezone(a){a._tz_cached=a.tz;if(a.tz in tz_aliases){a._tz_cached=tz_aliases[a.tz];console.log('TZ "'+a.tz+'" resolved as "'+a._tz_cached+'"')}if(moment.tz.zone(a._tz_cached)){people_loading--;
return}scheduleGoogleReq(a)}function scheduleGoogleReq(c){var a=(Date.now()-last_google_call_timestamp);var b=Math.max(0,(110-a));if(b==0){last_google_call_timestamp=Date.now();getTimezoneFromGoogleApis(c);
return}setTimeout(function(){scheduleGoogleReq(c)},b)}function getTimezoneFromGoogleApis(b){var a="https://maps.googleapis.com/maps/api/geocode/json?sensor=false&address="+encodeURIComponent(b.tz);callAjax(a,function(k){try{var c=JSON.parse(k);
if(c.status==="OK"){console.log("resp. is OK");var d=c.results;console.log(d);if(d.length>1){console.error('WARNING: Google found multiple matches. Try to be more specific at "'+b.tz+'"')}var i=d[0].geometry.location.lat;
var j=d[0].geometry.location.lng;var g=Math.floor(Date.now()/1000);var f="https://maps.googleapis.com/maps/api/timezone/json?location="+i+","+j+"&timestamp="+g+"&sensor=false";callAjax(f,function(n){try{var l=JSON.parse(n);
if(l.status==="OK"){console.log("Resolved TZ as "+l.timeZoneId);b._tz_cached=l.timeZoneId}else{b._valid=false}}catch(m){console.log(m);b._valid=false}people_loading--},function(e){console.log("FAIL tzAPI: "+e);
b._valid=false;people_loading--})}else{b._valid=false;people_loading--}}catch(h){console.log(h);b._valid=false;people_loading--}},function(c){console.log("FAIL geoAPI: "+c);b._valid=false;people_loading--
})}function getTimezoneForPerson(a){return a._tz_cached}function getTimeForPerson(b){var c=getTimezoneForPerson(b);if(c===false){return false}var a=moment().tz(c);return a.hour()*3600+a.minute()*60+a.second()
}var disc;var mouse_on_list;function init(){disc=document.getElementById("disc");buildClockMarks();updateTime();setInterval(updateTime,1000);loadPeopleArray(function(){updatePeople();setInterval(updatePeople,1000*10);
window.onfocus=function(){updatePeople();updateTime()}})}function buildClockMarks(){for(var a=0;a<24;a++){var c=document.createElement("div");c.classList.add("mark");c.classList.add("hour-"+a);if(a==0||a==6||a==12||a==18){c.classList.add("sixth")
}c.textContent=""+a;var b=hour2angle(a);positionAt(c,b,45);disc.appendChild(c)}}function updatePeople(){if(mouse_on_list){console.log("Mouse over list, not redrawing.");return}var a=document.querySelectorAll(".bullet, .person, .people-list");
for(var b in a){if(a.hasOwnProperty(b)){var c=a[b];if(!c||!c.parentNode){continue}c.parentNode.removeChild(c)}}buildPeople()}var last_time;function updateTime(){var c=moment();var a=c.format("H:mm");if(a!==last_time){var d=a.split(":");
document.getElementById("localtime").innerHTML=d[0]+'<span id="loctimecolon">:</span>'+d[1]}var b=(new Date()).getSeconds()%2;document.getElementById("loctimecolon").style.opacity=b?1:0}function buildPeople(){var a=[];
people.forEach(function(d){if(!d._valid){return}var b=getTimeForPerson(d);if(b===false){return}d._t=b;var c;a.some(function(e){if(Math.abs(e.t-b)<60){e.p.push(d);c=true;return true}});if(!c){a.push({t:b,p:[d]})
}});a.forEach(function(b){addPeopleAtTime(b.t,b.p)})}function addPeopleAtTime(g,r){var e;var f=r[0];var q=g/3600;var c=hour2angle(q);var p=Math.floor(c/45);var d=Math.floor(p/2);var o=d<2;var a=(d>0&&d<3);
var n=document.createElement("div");n.className="bullet";n.style.backgroundColor=f.color;positionAt(n,c,50.2);disc.appendChild(n);var h=document.createElement("div");h.classList.add("people-list");h.classList.add(a?"left":"right");
h.classList.add(o?"up":"down");h.classList.add("quad"+d);h.classList.add("oct"+p);var m=moment();var k=moment().tz(getTimezoneForPerson(f));e=mmtDayCompare(m,k);var j=(e==-1)?"day-prev":(e==1)?"day-next":null;
if(j!==null){h.classList.add(j);n.classList.add(j)}if(r.length>1){h.classList.add("multiple");h.classList.add("count-"+r.length)}for(e=0;e<r.length;e++){var l=r[e];var b=createPersonLabel(l);b.title=k.format("H:mm, MMM Do")+" — "+l.tz;
h.appendChild(b)}h.addEventListener("mouseover",function(){mouse_on_list=true});h.addEventListener("mouseout",function(){mouse_on_list=false});positionAt(h,c,53.5,p);disc.appendChild(h)}function createPersonLabel(b){var c=(b.name.indexOf("@")===0);
var a=document.createElement(c?"a":"span");a.classList.add("person-label");if(c){a.href="https://twitter.com/"+b.name;a.target="_blank"}a.style.color=b.color;a.textContent=b.name;return a};
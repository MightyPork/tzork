...
}var a=moment().tz(c);return a.hour()*3600+a.minute()*60+a.second()}var disc;function init(){disc=document.getElementById("disc");buildClockMarks();update();setInterval(update,1000*10);setInterval(changeColon,1000);
window.onfocus=update}function buildClockMarks(){for(var a=0;a<24;a++){var c=document.createElement("div");c.classList.add("mark");c.classList.add("hour-"+a);if(a==0||a==6||a==12||a==18){c.classList.add("sixth")
}c.textContent=""+a;var b=hour2angle(a);positionAt(c,b,45);disc.appendChild(c)}}function changeColon(){var a=(new Date()).getSeconds()%2;document.getElementById("loctimecolon").style.opacity=a?1:0}function update(){redrawPeople();
redrawTime()}function redrawPeople(){var a=document.querySelectorAll(".bullet, .person, .people-list");for(var b in a){if(a.hasOwnProperty(b)){var c=a[b];if(!c||!c.parentNode){continue}c.parentNode.removeChild(c)
}}buildPeople()}function redrawTime(){var a=moment();var b=a.format("H:mm").split(":");document.getElementById("localtime").innerHTML=b[0]+'<span id="loctimecolon">:</span>'+b[1];changeColon()}function buildPeople(){var a=[];
people.forEach(function(d){var b=getTimeForPerson(d);if(b===false){return}d._t=b;var c;a.some(function(e){if(Math.abs(e.t-b)<60){e.p.push(d);c=true;return true}});if(!c){a.push({t:b,p:[d]})}});a.forEach(function(b){console.log("Time "+b.t+", people #: "+b.p.length);
addPeopleAtTime(b.t,b.p)})}function addPeopleAtTime(c,b){var n,m;var d=b[0];var e=c/3600;var o=hour2angle(e);var k=Math.floor(o/45);var f=Math.floor(k/2);var s=f<2;var l=(f>0&&f<3);console.log("angle = "+o+", octant "+k+", quadrant "+f+", up "+s+", left "+l);
var q=document.createElement("div");q.className="bullet";q.style.backgroundColor=d.color;positionAt(q,o,50.2);disc.appendChild(q);var p=document.createElement("div");p.classList.add("people-list");p.classList.add(l?"left":"right");
p.classList.add(s?"up":"down");p.classList.add("quad"+f);p.classList.add("oct"+k);var a=moment();var h=moment().tz(getTimezoneForPerson(d));n=mmtDayCompare(a,h);var u=(n==-1)?"day-prev":(n==1)?"day-next":null;
if(u!==null){p.classList.add(u);q.classList.add(u)}if(b.length>1){p.classList.add("multiple");p.classList.add("count-"+b.length)}p.style.color=d.color;for(n=0;n<b.length;n++){var g=b[n];var r=createPersonLabel(g);
r.title=h.format("H:mm, MMM Do")+"; "+g.tz;p.appendChild(r)}positionAt(p,o,53.5,k);disc.appendChild(p)}function createPersonLabel(b){var c=(b.name.indexOf("@")===0);var a=document.createElement(c?"a":"span");
a.classList.add("person-label");if(c){a.href="https://twitter.com/"+b.name;a.classList.add("twitter-link");a.target="_blank"}a.style.color=b.color;a.textContent=b.name;return a};
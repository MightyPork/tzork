...
addPeopleAtTime(b.t,b.p)})}function addPeopleAtTime(g,r){var e,d;var f=r[0];var q=g/3600;var c=hour2angle(q);var p=Math.floor(c/45);var b=Math.floor(p/2);var o=b<2;var a=(b>0&&b<3);console.log("angle = "+c+", octant "+p+", quadrant "+b+", up "+o+", left "+a);
...
h.classList.add(o?"up":"down");h.classList.add("quad"+b);h.classList.add("oct"+p);var m=moment();var l=moment().tz(getTimezoneForPerson(f));e=mmtDayCompare(m,l);var k=(e==-1)?"day-prev":(e==1)?"day-next":null;
if(k!==null){h.classList.add(k);n.classList.add(k)}if(r.length>1){h.classList.add("multiple");h.classList.add("count-"+r.length)}h.title=l.format("H:mm, MMM Do YYYY");h.style.color=f.color;for(e=0;e<r.length;
e++){h.appendChild(createPersonLabel(r[e]))}positionAt(h,c,53.5,p);disc.appendChild(h)}function createPersonLabel(b){var c=(b.name.indexOf("@")===0);var a=document.createElement(c?"a":"span");a.classList.add("person-label");
if(c){a.href="https://twitter.com/"+b.name;a.classList.add("twitter-link");a.target="_blank"}a.style.color=b.color;a.textContent=b.name;return a};
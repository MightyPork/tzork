...
var people=[{name:"@AntonvonRaumer",tz:"Germany",color:"#FF8585"},{name:"@ciba43",tz:"Latvia",color:"#FE2F77"},{name:"@coolsquid_",tz:"Norway",color:"#079DE9"},{name:"@Creeper32605",tz:"Denmark",color:"#C8F080"},{name:"@Deli731234",tz:"USA, Pacific Time",color:"#A659E2"},{name:"@eevblog",tz:"Australia, Sydney",color:"#E63E3F"},{name:"@elakdawalla",tz:"USA, Pacific Time",color:"#C49B6D"},{name:"@iamSethD",tz:"USA, Eastern Time",color:"#ED2222"},{name:"@ImReble548",tz:"USA, Mountain Time",color:"#69E79E"},{name:"@Jaredlll08",tz:"South Africa",color:"#F86752"},{name:"@JesperJacoben",tz:"Denmark",color:"skyblue"},{name:"@lastofavari",tz:"Belarus",color:"#05CDC0"},{name:"@ljfa2",tz:"Germany",color:"#78FCFF"},{name:"@Locercus",tz:"Denmark",color:"#32CD32"},{name:"@Lunatrius",tz:"Slovenia",color:"#00CCFF"},{name:"@MightyPork",tz:"Czech Republic",color:"#FFAD00"},{name:"@Rafmaninoff",tz:"Spain",color:"#00CC00"},{name:"@TheBadFame",tz:"Mexico",color:"#329AFF"},{name:"@Vangoule273",tz:"United Kingdom",color:"#00FF2C"},{name:"@xTordX",tz:"Romania",color:"#AE1AF7"}];
...
}}}};c.onError=function(){if(typeof a=="function"){a(-1)}};c.send()}if(!Date.now){Date.now=function(){return new Date().getTime()}}function hour2angle(c){var b=(18-c)*15;while(b<0){b+=360}return b}function positionAt(a,d,f,b){if(typeof b==="undefined"){b=0
...
break;case 5:a.style.right=(50-c)+"%";a.style.bottom=(50+e)+"%";break;case 6:a.style.left=(50+c)+"%";a.style.bottom=(50+e)+"%";break}}function mmtDayCompare(a,e){var f=a.dayOfYear();var c=a.year();var d=e.dayOfYear();
var b=e.year();if(b<c){return -1}else{if(b>c){return 1}else{if(d<f){return -1}else{if(d>f){return 1}}}}return 0}(function(){function g(j){var h=localStorage.people;if(typeof h!="undefined"){try{people=JSON.parse(h)
}catch(i){console.error("Error when parsing people array from localstorage",i)}}c(j)}var f=0;function c(i){f=0;people.forEach(function(j){j._valid=true;var k=false;["name","color","tz"].forEach(function(l){if(typeof(j[l])=="undefined"){console.error('Missing "'+l+'" field in person object',j);
k=true}});if(k){j._valid=false;return}f++;b(j)});var h=function(){if(f<=0){console.log("LOADING DONE");i();return}setTimeout(h,10)};h()}var e=0;function b(h){h._tz_cached=h.tz;if(h.tz in tz_aliases){h._tz_cached=tz_aliases[h.tz];
...
}setTimeout(function(){a(j)},i)}function d(i){var h="https://maps.googleapis.com/maps/api/geocode/json?sensor=false&address="+encodeURIComponent(i.tz);callAjax(h,function(q){try{var j=JSON.parse(q);if(j.status==="OK"){console.log("resp. is OK");
var k=j.results;console.log(k);if(k.length>1){console.error('WARNING: Google found multiple matches. Try to be more specific at "'+i.tz+'"')}var o=k[0].geometry.location.lat;var p=k[0].geometry.location.lng;
var m=Math.floor(Date.now()/1000);var l="https://maps.googleapis.com/maps/api/timezone/json?location="+o+","+p+"&timestamp="+m+"&sensor=false";callAjax(l,function(t){try{var r=JSON.parse(t);if(r.status==="OK"){console.log("Resolved TZ as "+r.timeZoneId);
i._tz_cached=r.timeZoneId}else{i._valid=false}}catch(s){console.log(s);i._valid=false}f--},function(r){console.log("FAIL tzAPI: "+r);i._valid=false;f--})}else{i._valid=false;f--}}catch(n){console.log(n);
i._valid=false;f--}},function(j){console.log("FAIL geoAPI: "+j);i._valid=false;f--})}window.loadPeopleArray=g;window.resolvePeopleTimezones=c})();function getTimezoneForPerson(a){return a._tz_cached}function getTimeForPerson(b){var c=getTimezoneForPerson(b);
if(c===false){return false}var a=moment().tz(c);return a.hour()*3600+a.minute()*60+a.second()}function openSetupDialog(){document.getElementById("people_error").textContent="";var a=document.getElementById("people_json");
var b=JSON.parse(JSON.stringify(people));b.forEach(function(d){delete d._t;delete d._valid;delete d._tz_cached});a.value=JSON.stringify(b,null,"\t");var c=document.getElementById("setup_dialog");c.style.display="block";
setTimeout(function(){c.style.opacity=1},1)}function submitPeopleEdit(c){switch(c){case"defaults":localStorage.people=JSON.stringify(people_orig);init();hideSetupModal();break;case"close":hideSetupModal();
break;case"save":try{var b=document.getElementById("people_json");var a=JSON.parse(b.value);a.forEach(function(e){if(typeof e.name=="undefined"){throw new SyntaxError('Missing "name"')}if(typeof e.tz=="undefined"){throw new SyntaxError('Missing "tz" for '+e.name)
}if(typeof e.color=="undefined"){throw new SyntaxError('Missing "color" for '+e.name)}});localStorage.people=JSON.stringify(a);init();hideSetupModal()}catch(d){var f=document.getElementById("people_error");
if(d.message.match(/^Unexpected token [,\]}]/g)){f.textContent="Syntax error: trailing comma ?"}else{if(d.message.match(/^Unexpected string/g)){f.textContent="Syntax error: missing comma ?"}else{f.textContent="Syntax error: "+d.message
}}console.log("Error in user-entered JSON",d)}break}}function hideSetupModal(){var a=document.getElementById("setup_dialog");a.style.opacity=0;setTimeout(function(){a.style.display="none"},500)}var people_orig;
(function(){var m;var a;var g;var j;var b;var f=true;function k(){var p;if(f){resolvePeopleTimezones(function(){people_orig=JSON.parse(JSON.stringify(people));people_orig.forEach(function(r){delete r._t;
delete r._valid;delete r._tz_cached})});var n=document.getElementsByTagName("textarea");var q=n.length;for(p=0;p<q;p++){n[p].onkeydown=function(t){if(t.keyCode==9||t.which==9){t.preventDefault();var r=this.selectionStart;
this.value=this.value.substring(0,this.selectionStart)+"\t"+this.value.substring(this.selectionEnd);this.selectionEnd=r+1}}}f=false}var o=document.querySelectorAll(".mark, .bullet, .person-label, .people-list");
for(p=0;p<o.length;p++){o[p].parentNode.removeChild(o[p])}clearInterval(b);clearInterval(j);g=document.getElementById("disc");i();h();j=setInterval(h,1000);loadPeopleArray(function(){e();b=setInterval(e,1000*10);
window.onFocus=function(){e();h()};var r=document.getElementById("setup_btn");r.addEventListener("click",function(){openSetupDialog()})})}function i(){for(var n=0;n<24;n++){var p=document.createElement("div");
p.classList.add("mark");p.classList.add("hour-"+n);if(n==0||n==6||n==12||n==18){p.classList.add("sixth")}p.textContent=""+n;var o=hour2angle(n);positionAt(p,o,45);g.appendChild(p)}}function h(){var p=moment();
var n=p.format("H:mm");if(n!==a){a=n;var q=n.split(":");document.getElementById("localtime").innerHTML=q[0]+'<span id="loctimecolon">:</span>'+q[1]}var o=(new Date()).getSeconds()%2;document.getElementById("loctimecolon").style.opacity=o?1:0
}function e(){if(m){console.log("Mouse over list, not redrawing.");return}var n=document.querySelectorAll(".bullet, .person, .people-list");for(var o in n){if(n.hasOwnProperty(o)){var p=n[o];if(!p||!p.parentNode){continue
}p.parentNode.removeChild(p)}}l()}function l(){var n=[];people.forEach(function(q){if(!q._valid){return}var o=getTimeForPerson(q);if(o===false){return}q._t=o;var p;n.some(function(r){if(Math.abs(r.t-o)<60){r.p.push(q);
p=true;return true}});if(!p){n.push({t:o,p:[q]})}});n.forEach(function(o){d(o.t,o.p)})}function d(u,E){var r;var s=E[0];var D=u/3600;var p=hour2angle(D);var C=Math.floor(p/45);var q=Math.floor(C/2);var B=q<2;
var n=(q>0&&q<3);var A=document.createElement("div");A.className="bullet";A.style.backgroundColor=s.color;positionAt(A,p,50.2);g.appendChild(A);var v=document.createElement("div");v.classList.add("people-list");
v.classList.add(n?"left":"right");v.classList.add(B?"up":"down");v.classList.add("quad"+q);v.classList.add("oct"+C);var z=moment();var x=moment().tz(getTimezoneForPerson(s));r=mmtDayCompare(z,x);var w=(r==-1)?"day-prev":(r==1)?"day-next":null;
if(w!==null){v.classList.add(w);A.classList.add(w)}if(E.length>1){v.classList.add("multiple");v.classList.add("count-"+E.length)}for(r=0;r<E.length;r++){var y=E[r];var o=c(y);o.title=x.format("H:mm, MMM Do")+" — "+y.tz;
v.appendChild(o)}v.addEventListener("mouseover",function(){m=true});v.addEventListener("mouseout",function(){m=false});positionAt(v,p,53.5,C);g.appendChild(v)}function c(o){var p=(o.name.indexOf("@")===0);
var n=document.createElement(p?"a":"span");n.classList.add("person-label");if(p){n.href="https://twitter.com/"+o.name;n.target="_blank"}n.style.color=o.color;n.textContent=o.name;return n}window.updatePeople=e;
window.updateTime=h;window.init=k})();
...
var people=[{name:"@MightyPork",tz:"Czech Republic",color:"#EAF425"},{name:"@lastofavari",tz:"Belarus",color:"#4ECEFF"},{name:"@Locercus",tz:"Denmark",color:"lime"},{name:"@Creeper32605",tz:"Denmark",color:"teal"},{name:"@JesperJacoben",tz:"Denmark",color:"skyblue"},{name:"@Jaredlll08",tz:"South Africa",color:"orange"},{name:"@ciba43",tz:"Latvia",color:"cyan"},{name:"@coolsquid_",tz:"Norway",color:"purple"},{name:"@TheBadFame",tz:"Mexico",color:"#329AFF"},{name:"@ImReble548",tz:"USA, Mountain Time",color:"#69E79E"},{name:"@Deli731234",tz:"USA, Pacific Time",color:"#F57126"},{name:"@eevblog",tz:"Australia, Sydney",color:"#E63E3F"}];
function hour2angle(a){return(18-a)*15}function positionAt(b,d,f,a){if(typeof a==="undefined"){a=0}var c=f*Math.cos((d/180)*Math.PI);var e=f*Math.sin((d/180)*Math.PI);switch(a){case 0:b.style.left=50+c+"%";
b.style.top=50-e+"%";break;case 1:b.style.left=50+c+"%";b.style.top=50-e+"%";break;case 2:b.style.left=50+c+"%";b.style.top=50-e+"%";break;case 3:b.style.left=50+c+"%";b.style.top=50-e+"%";break}}function mmtDayCompare(a,e){var f=a.dayOfYear();
var c=a.year();var d=e.dayOfYear();var b=e.year();if(b<c){return -1}else{if(b>c){return 1}else{if(d<f){return -1}else{if(d>f){return 1}}}}return 0}function resolveTZ(b){if(b in tz_aliases){var a=b;b=tz_aliases[b];
console.log('TZ "'+a+'" resolved as "'+b+'"')}if(!moment.tz.zone(b)){return false}return b}function getTimezoneForPerson(a){var b=a._tz_cached;if(!b){b=a._tz_cached=resolveTZ(a.tz)}if(b===false){console.error("Invalid timezone for "+a.name);
return false}return b}function getTimeForPerson(b){var c=getTimezoneForPerson(b);if(c===false){return false}var a=moment().tz(c);return a.hour()*3600+a.minute()*60+a.second()}var disc;function init(){disc=document.getElementById("disc");
buildClockMarks();update();setInterval(update,1000*10);setInterval(changeColon,1000);window.onfocus=update}function buildClockMarks(){for(var a=0;a<24;a++){var c=document.createElement("div");c.classList.add("mark");
c.classList.add("hour-"+a);if(a==0||a==6||a==12||a==18){c.classList.add("sixth")}c.textContent=""+a;var b=hour2angle(a);positionAt(c,b,45);disc.appendChild(c)}}function changeColon(){var a=(new Date()).getSeconds()%2;
document.getElementById("loctimecolon").style.opacity=a?1:0}function update(){redrawPeople();redrawTime()}function redrawPeople(){var a=document.querySelectorAll(".bullet, .person, .people-list");for(var b in a){if(a.hasOwnProperty(b)){var c=a[b];
...
}});if(!c){a.push({t:b,p:[d]})}});a.forEach(function(b){console.log("Time "+b.t+", people #: "+b.p.length);addPeopleAtTime(b.t,b.p)})}function addPeopleAtTime(e,l){var d=l[0];var k=e/3600;var b=hour2angle(k);
var j=document.createElement("div");j.className="bullet";j.style.backgroundColor=d.color;positionAt(j,b,50.2);disc.appendChild(j);var a=document.createElement("div");a.classList.add("people-list");a.classList.add((k<12)?"left":"right");
a.classList.add((k<6||k>18)?"down":"up");var h=moment();var g=moment().tz(getTimezoneForPerson(d));var c=mmtDayCompare(h,g);var f=(c==-1)?"day-prev":(c==1)?"day-next":null;if(f!==null){a.classList.add(f);
j.classList.add(f)}a.innerHTML=d.name+"<br>hello";if(l.length>1){a.innerHTML+="<br> + "+(l.length-1)}a.title=g.format("H:mm, MMM Do YYYY");a.style.color=d.color;positionAt(a,b,51.5);disc.appendChild(a)
}function addPerson(e){var f=e._t;var l=(f/3600);var c=hour2angle(l);var k=document.createElement("div");k.className="bullet";k.style.backgroundColor=e.color;positionAt(k,c,50.2);disc.appendChild(k);var b=(e.name.indexOf("@")===0);
var a=document.createElement(b?"a":"span");if(b){a.href="https://twitter.com/"+e.name;a.classList.add("twitter-link")}a.classList.add("person");a.classList.add((l<12)?"left":"right");var j=moment();var h=moment().tz(getTimezoneForPerson(e));
var d=mmtDayCompare(j,h);var g=(d==-1)?"day-prev":(d==1)?"day-next":null;if(g!==null){a.classList.add(g);k.classList.add(g)}a.textContent=e.name;a.title=h.format("H:mm, MMM Do YYYY")+" ("+e.tz+")";a.style.color=e.color;
positionAt(a,c,53.5);disc.appendChild(a)};
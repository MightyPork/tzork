...
var people=[{name:"@MightyPork",tz:"Czech Republic",color:"#FFAD00"},{name:"@lastofavari",tz:"Belarus",color:"#05CDC0"},{name:"@Locercus",tz:"Denmark",color:"#32CD32"},{name:"@Creeper32605",tz:"Denmark",color:"#C8F080"},{name:"@JesperJacoben",tz:"Denmark",color:"skyblue"},{name:"@Jaredlll08",tz:"South Africa",color:"#F86752"},{name:"@ciba43",tz:"Latvia",color:"#FE2F77"},{name:"@coolsquid_",tz:"Norway",color:"#079DE9"},{name:"@TheBadFame",tz:"Mexico",color:"#329AFF"},{name:"@ImReble548",tz:"USA, Mountain Time",color:"#69E79E"},{name:"@Deli731234",tz:"USA, Pacific Time",color:"#A659E2"},{name:"@iamSethD",tz:"USA, Eastern Time",color:"#ED2222"},{name:"@eevblog",tz:"Australia, Sydney",color:"#E63E3F"},{name:"@AntonvonRaumer",tz:"Germany",color:"#FF8585"},{name:"@elakdawalla",tz:"USA, Pacific Time",color:"#C49B6D"},{name:"@Vangoule273",tz:"United Kingdom",color:"#FF0095"},{name:"@xTordX",tz:"Romania",color:"#AE1AF7"},{name:"@ljfa2",tz:"Germany",color:"#78FCFF"},{name:"@Rafmaninoff",tz:"Spain",color:"#00CC00"}];
...
}function resolveTZ(b){if(b in tz_aliases){var a=b;b=tz_aliases[b]}if(!moment.tz.zone(b)){return false}return b}function getTimezoneForPerson(a){var b=a._tz_cached;if(!b){b=a._tz_cached=resolveTZ(a.tz)
}if(b===false){console.error("Invalid timezone for "+a.name);return false}return b}function getTimeForPerson(b){var c=getTimezoneForPerson(b);if(c===false){return false}var a=moment().tz(c);return a.hour()*3600+a.minute()*60+a.second()
}var disc;var mouse_hovering_list;function init(){disc=document.getElementById("disc");buildClockMarks();update();setInterval(update,1000*10);setInterval(changeColon,1000);window.onfocus=update}function buildClockMarks(){for(var a=0;
a<24;a++){var c=document.createElement("div");c.classList.add("mark");c.classList.add("hour-"+a);if(a==0||a==6||a==12||a==18){c.classList.add("sixth")}c.textContent=""+a;var b=hour2angle(a);positionAt(c,b,45);
disc.appendChild(c)}}function changeColon(){var a=(new Date()).getSeconds()%2;document.getElementById("loctimecolon").style.opacity=a?1:0}function update(){redrawPeople();redrawTime()}function redrawPeople(){if(mouse_hovering_list){console.log("Mouse over list, not redrawing.");
return}var a=document.querySelectorAll(".bullet, .person, .people-list");for(var b in a){if(a.hasOwnProperty(b)){var c=a[b];if(!c||!c.parentNode){continue}c.parentNode.removeChild(c)}}buildPeople()}function redrawTime(){var a=moment();
var b=a.format("H:mm").split(":");document.getElementById("localtime").innerHTML=b[0]+'<span id="loctimecolon">:</span>'+b[1];changeColon()}function buildPeople(){var a=[];people.forEach(function(d){var b=getTimeForPerson(d);
if(b===false){return}d._t=b;var c;a.some(function(e){if(Math.abs(e.t-b)<60){e.p.push(d);c=true;return true}});if(!c){a.push({t:b,p:[d]})}});a.forEach(function(b){addPeopleAtTime(b.t,b.p)})}function addPeopleAtTime(f,r){var d;
var e=r[0];var q=f/3600;var b=hour2angle(q);var p=Math.floor(b/45);var c=Math.floor(p/2);var o=c<2;var a=(c>0&&c<3);var m=document.createElement("div");m.className="bullet";m.style.backgroundColor=e.color;
positionAt(m,b,50.2);disc.appendChild(m);var g=document.createElement("div");g.classList.add("people-list");g.classList.add(a?"left":"right");g.classList.add(o?"up":"down");g.classList.add("quad"+c);g.classList.add("oct"+p);
var l=moment();var j=moment().tz(getTimezoneForPerson(e));d=mmtDayCompare(l,j);var h=(d==-1)?"day-prev":(d==1)?"day-next":null;if(h!==null){g.classList.add(h);m.classList.add(h)}if(r.length>1){g.classList.add("multiple");
g.classList.add("count-"+r.length)}g.style.color=e.color;for(d=0;d<r.length;d++){var k=r[d];var n=createPersonLabel(k);n.title=j.format("H:mm, MMM Do")+" — "+k.tz;g.appendChild(n)}positionAt(g,b,53.5,p);
disc.appendChild(g);g.addEventListener("mouseover",function(i){mouse_hovering_list=true});g.addEventListener("mouseout",function(i){mouse_hovering_list=false})}function createPersonLabel(b){var c=(b.name.indexOf("@")===0);

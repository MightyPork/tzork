...
return a.hour()*3600+a.minute()*60+a.second()}function openSetupDialog(){document.getElementById("people_error").textContent="";var a=JSON.parse(JSON.stringify(people));a.forEach(function(c){delete c._t;
delete c._valid;delete c._tz_cached});var b=document.getElementById("setup_dialog");b.style.display="block";setTimeout(function(){b.style.opacity=1},1)}function submitPeopleEdit(c){switch(c){case"defaults":localStorage.people=JSON.stringify(people_orig);
init();hideSetupModal();break;case"close":hideSetupModal();break;case"save":try{var b=document.getElementById("people_json");var a=JSON.parse(b.value);a.forEach(function(e){if(typeof e.name=="undefined"){throw new SyntaxError('Missing "name"')
}if(typeof e.tz=="undefined"){throw new SyntaxError('Missing "tz" for '+e.name)}if(typeof e.color=="undefined"){throw new SyntaxError('Missing "color" for '+e.name)}});localStorage.people=JSON.stringify(a);
init();hideSetupModal()}catch(d){var f=document.getElementById("people_error");if(d.message.match(/^Unexpected token [,\]}]/g)){f.textContent="Syntax error: trailing comma ?"}else{if(d.message.match(/^Unexpected string/g)){f.textContent="Syntax error: missing comma ?"
}else{f.textContent="Syntax error: "+d.message}}console.log("Error in user-entered JSON",d)}break}}function hideSetupModal(){var a=document.getElementById("setup_dialog");a.style.opacity=0;setTimeout(function(){a.style.display="none"
},500)}var people_orig;(function(){var m;var a;var g;var j;var b;var f=true;function k(){var p;if(f){resolvePeopleTimezones(function(){people_orig=JSON.parse(JSON.stringify(people));people_orig.forEach(function(r){delete r._t;
...
var n=p.format("H:mm");if(n!==a){a=n;var q=n.split(":");document.getElementById("localtime").innerHTML=q[0]+'<span id="loctimecolon">:</span>'+q[1]}var o=(new Date()).getSeconds()%2;document.getElementById("loctimecolon").style.visibility=o?"visible":"hidden"
...
if(w!==null){v.classList.add(w);A.classList.add(w)}if(E.length>1){v.classList.add("multiple");v.classList.add("count-"+E.length)}for(r=0;r<E.length;r++){var y=E[r];var o=c(y);o.title=x.format("H:mm, MMM Do")+" — "+y._tz_cached;

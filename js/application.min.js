...
}var disc;var mouse_on_list;function init(){disc=document.getElementById("disc");buildClockMarks();loadPeopleArray(function(){update();setInterval(update,1000*10);setInterval(changeColon,1000);window.onfocus=update
})}function buildClockMarks(){for(var a=0;a<24;a++){var c=document.createElement("div");c.classList.add("mark");c.classList.add("hour-"+a);if(a==0||a==6||a==12||a==18){c.classList.add("sixth")}c.textContent=""+a;
var b=hour2angle(a);positionAt(c,b,45);disc.appendChild(c)}}function changeColon(){var a=(new Date()).getSeconds()%2;document.getElementById("loctimecolon").style.opacity=a?1:0}function update(){redrawPeople();
redrawTime()}function redrawPeople(){if(mouse_on_list){console.log("Mouse over list, not redrawing.");return}var a=document.querySelectorAll(".bullet, .person, .people-list");for(var b in a){if(a.hasOwnProperty(b)){var c=a[b];
if(!c||!c.parentNode){continue}c.parentNode.removeChild(c)}}buildPeople()}function redrawTime(){var a=moment();var b=a.format("H:mm").split(":");document.getElementById("localtime").innerHTML=b[0]+'<span id="loctimecolon">:</span>'+b[1];
changeColon()}function buildPeople(){var a=[];people.forEach(function(d){if(!d._valid){return}var b=getTimeForPerson(d);if(b===false){return}d._t=b;var c;a.some(function(e){if(Math.abs(e.t-b)<60){e.p.push(d);
c=true;return true}});if(!c){a.push({t:b,p:[d]})}});a.forEach(function(b){addPeopleAtTime(b.t,b.p)})}function addPeopleAtTime(g,r){var e;var f=r[0];var q=g/3600;var c=hour2angle(q);var p=Math.floor(c/45);
var d=Math.floor(p/2);var o=d<2;var a=(d>0&&d<3);var n=document.createElement("div");n.className="bullet";n.style.backgroundColor=f.color;positionAt(n,c,50.2);disc.appendChild(n);var h=document.createElement("div");
h.classList.add("people-list");h.classList.add(a?"left":"right");h.classList.add(o?"up":"down");h.classList.add("quad"+d);h.classList.add("oct"+p);var m=moment();var k=moment().tz(getTimezoneForPerson(f));
e=mmtDayCompare(m,k);var j=(e==-1)?"day-prev":(e==1)?"day-next":null;if(j!==null){h.classList.add(j);n.classList.add(j)}if(r.length>1){h.classList.add("multiple");h.classList.add("count-"+r.length)}for(e=0;
e<r.length;e++){var l=r[e];var b=createPersonLabel(l);b.title=k.format("H:mm, MMM Do")+" — "+l.tz;h.appendChild(b)}h.addEventListener("mouseover",function(){mouse_on_list=true});h.addEventListener("mouseout",function(){mouse_on_list=false
});positionAt(h,c,53.5,p);disc.appendChild(h)}function createPersonLabel(b){var c=(b.name.indexOf("@")===0);var a=document.createElement(c?"a":"span");a.classList.add("person-label");if(c){a.href="https://twitter.com/"+b.name;
a.target="_blank"}a.style.color=b.color;a.textContent=b.name;return a};